...
describe('debug log', () => {
  it('still writes the flat line to storage and hands it to takePendingLines', async () => {
    dlog('apply', '4001', { status: 'applied' });
    const pending = await takePendingLines();
    ...
    expect(await takePendingLines()).toEqual([]); // pending list is drained, the history is not
  ...
  it('an observe event is written once, not twice, and still reaches the flat log', async () => {
    ...
    expect((await takePendingLines()).join()).toMatch(/run run started on datadog/);
  });
});
